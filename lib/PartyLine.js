				...
				if (other!=transport)
				{
					...
					if (outgoing.getMedia()=="video")
					{
						...
						const timer = setInterval(()=>{
							//Get active lauers
							const stats = track.getActiveLayers();
							//send event
							other.tm.event("layers",{
								streamId : outgoingStreamId,
								stats	 : stats
							});
							//Switch to highest layer on first stat
							if (first && stats.layers.length)
							{
								//Get transponder
								const transponder = outgoing.getTransponder();
								//Select top encoding
								transponder.selectEncoding(stats.layers[0].encodingId);
								//No more
								first = false;
							}
						},1000);
